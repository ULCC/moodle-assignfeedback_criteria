...
/**
 * External API for the criteria feedback plugin.
 *
...
/**
 * Library class for the criteria feedback plugin external API functions.
 *
 * @package   assignfeedback_criteria
 * @copyright 2017 Lancaster University {@link http://www.lancaster.ac.uk/}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @author    Tony Butler <a.butler4@lancaster.ac.uk>
 */
class assignfeedback_criteria_external extends external_api {
    ...
    public static function get_criteria($contextid, $criteriasetid) {
        ...
        require_once($CFG->dirroot . '/mod/assign/feedback/criteria/locallib.php');
        ...
        $feedback = new assign_feedback_criteria($assignment, 'criteria');
        ...
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    ...
                    'descriptionpos' => new external_value(PARAM_RAW, 'The criterion positive description', VALUE_OPTIONAL),
					 'descriptionneg' => new external_value(PARAM_RAW, 'The criterion negative description', VALUE_OPTIONAL)
    ...
    public static function get_criteriasets($contextid, $includeshared) {
        ...
        $context = self::get_context_from_params(array('contextid' => $contextid));
        self::validate_context($context);
        if (!$includeshared && !has_capability('assignfeedback/criteria:manageowncriteriasets', $context)) {
            throw new moodle_exception('nopermissionstomanage', 'assignfeedback_criteria');
        }
    ...
    public static function save_criteriaset($contextid, $name, $criteria, $shared) {
       ...
       // self::validate_parameters(self::save_criteriaset_parameters(), $parameters);
        $context = self::get_context_from_params(array('contextid' => $contextid));
        self::validate_context($context);
        if (!has_capability('assignfeedback/criteria:manageowncriteriasets', $context)) {
            throw new moodle_exception('nopermissionstosave', 'assignfeedback_criteria');
        }
        ...
        if (empty($name)) {
            return array(
                'hide'  => false,
                'title' => get_string('criteriasetnonametitle', 'assignfeedback_criteria'),
                'body'  => get_string('criteriasetnoname', 'assignfeedback_criteria'),
                'label' => get_string('continue')
            );
        }
        // Validate global uniqueness of criteria set name provided.
        if ($DB->record_exists('assignfeedback_criteria_cs', array('name_lc' => strtolower($name)))) {
            return array(
                'hide'  => false,
                'title' => get_string('criteriasetnameusedtitle', 'assignfeedback_criteria'),
                'body'  => get_string('criteriasetnameused', 'assignfeedback_criteria', ucfirst($name)),
                'label' => get_string('continue')
            );
        }
        ...
        if ($feedback->save_criteria_set($name, $criteria, $shared)) {
            return array(
                'hide'  => true,
                'title' => get_string('criteriasetsaved', 'assignfeedback_criteria'),
                'body'  => get_string('criteriasetsavedsuccess', 'assignfeedback_criteria', ucfirst($name)),
                'label' => get_string('ok')
            );
        }
        ...
        return array(
            ...
            'body'  => get_string('criteriasetnotsaved', 'assignfeedback_criteria'),
            'label' => get_string('continue')
        );
    }
    ...
    public static function update_criteriaset($contextid, $criteriasetid, $updates) {
        ...
        self::validate_context($context);
        if (!has_capability('assignfeedback/criteria:manageowncriteriasets', $context)) {
            throw new moodle_exception('nopermissionstoupdate', 'assignfeedback_criteria');
        }
        foreach ($updates as $key => $value) {
            if ($key == 'shared' && $value == true && !has_capability('assignfeedback/criteria:publishcriteriasets', $context)) {
                throw new moodle_exception('nopermissionstopublish', 'assignfeedback_criteria');
            }
            ...
            if ($key == 'name') {
                ...
                if ($criteriaset = $DB->get_record('assignfeedback_criteria_cs', array('name_lc' => $namelc), 'id')) {
                    if ($criteriaset->id != $criteriasetid) {
                        return array(
                            'success' => false,
                            'title'   => get_string('criteriasetnameusedtitle', 'assignfeedback_criteria'),
                            'body'    => get_string('criteriasetnameused', 'assignfeedback_criteria', $value),
                            'label'   => get_string('continue')
                        );
                    }
                }
        ...
        if ($feedback->update_criteria_set($criteriasetid, $updates)) {
            return array(
                'success' => true,
                'title'   => get_string('criteriasetupdated', 'assignfeedback_criteria'),
                'body'    => get_string('criteriasetupdatedsuccess', 'assignfeedback_criteria'),
                'label'   => get_string('ok')
            );
        }
        ...
        return array(
            'success' => false,
            'title'   => get_string('error'),
            'body'    => get_string('criteriasetnotupdated', 'assignfeedback_criteria'),
            'label'   => get_string('continue')
        );
    }
    ...
    public static function delete_criteriaset($contextid, $criteriasetid) {
        ...
        self::validate_context($context);
        if (!has_capability('assignfeedback/criteria:manageowncriteriasets', $context)) {
            throw new moodle_exception('nopermissionstodelete', 'assignfeedback_criteria');
        }
