...
/**
 * Launches a modal dialogue that enables users to save the configured criteria set for the criteria feedback plugin.
 *
 * See template: assignfeedback_criteria/criteriasetsave
 *
 * @module     assignfeedback_criteria/criteriasetsave
    ...
    function($, ajax, notification, str, templates, ModalFactory, ModalEvents) {
        var criteriaSetSave = {
            ...
            init: function(contextId, canPublish) {
                str.get_string('criteriasetsave', 'assignfeedback_criteria').done(function(title) {
                    var context = {
                        canPublish: canPublish
                    };
                    var trigger = $('#id_assignfeedback_criteria_critsetsave');
                    ModalFactory.create({
                        title: title,
                        body: templates.render('assignfeedback_criteria/criteriasetsave', context),
        ...
        function saveSet(modal, contextId) {
            ...
            var criteria = [];
            modalNode.parent().find('[id^="id_assignfeedback_criteria_critname"]').each(function() {
                if ($(this).val().trim().length) {
                    var descNode = $(this).parent().parent().next().find('[name^="assignfeedback_criteria_critdesc"]');
                    if (!descNode.length) {
            ...
            var request = ajax.call([{
                methodname: 'assignfeedback_criteria_save_criteriaset',
                args: {
                    contextid: contextId,
                    name: name,
                    criteria: criteria,
            ...
            request[0].done(function(response) {
                if (response.hide === true) {
                    modal.hide();
                    $('#id_assignfeedback_criteria_critsetsmanage').prop('disabled', false);
                }
