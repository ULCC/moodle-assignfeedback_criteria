...
/**
 * Launches a modal dialogue that enables users to manage their criteria sets for the criteria feedback plugin.
 *
 * See template: assignfeedback_criteria/criteriasets
 *               assignfeedback_criteria/criteriasets_footer
 *
 * @module     assignfeedback_criteria/criteriasets
 * @class      criteriasets
    ...
    function($, ajax, notification, str, templates, ModalFactory, ModalEvents) {
        var criteriaSets = {
            ...
            init: function(contextId, manage, canPublish, ownedSets, sharedSets) {
                str.get_strings([
                    {key: 'criteriasetssaved', component: 'assignfeedback_criteria'},
                    {key: 'criteriasetsmanage', component: 'assignfeedback_criteria'}
                ]).done(function(s) {
                    var context = {
                        contextId: contextId,
                        manage: manage,
                    ...
                    var title = s[0],
                        trigger = $('#id_assignfeedback_criteria_critset');
                    if (manage) {
                        title = s[1];
                        trigger = $('#id_assignfeedback_criteria_critsetsmanage');
                    }
                    templates.render('assignfeedback_criteria/criteriasets_footer', footerContext).done(function(footer) {
                        ModalFactory.create({
                            title: title,
                            body: templates.render('assignfeedback_criteria/criteriasets', context),
                            footer: footer,
                            large: false
                ...
                if (!manage) {
                    var setButton = $('#id_assignfeedback_criteria_critset');
        ...
        function refreshSets(modal, contextId, manage, canPublish) {
            ...
            var request = ajax.call([{
                methodname: 'assignfeedback_criteria_get_criteriasets',
                args: {
                    contextid: contextId,
            ...
            request[0].done(function(response) {
                ...
                templates.render('assignfeedback_criteria/criteriasets', context).done(function(html, js) {
