...
namespace assignfeedback_criteria\privacy;
...
class provider implements metadataprovider, assignfeedback_provider {
    ...
    public static function get_metadata(collection $collection) : collection {
        $data = [
            ...
            'criteriondescpos' => 'privacy:metadata:critdescpurpose'
			'criteriondescneg' => 'privacy:metadata:critdescpurpose'
        ];
        $collection->add_database_table('assignfeedback_criteria', $data, 'privacy:metadata:tablesummary');
        return $collection;
    }
    ...
    public static function export_feedback_user_data(assign_plugin_request_data $exportdata) {
        ...
        $subcontext = array_merge($exportdata->get_subcontext(), [get_string('privacy:path', 'assignfeedback_criteria')]);
        $assign = $exportdata->get_assign();
        $plugin = $assign->get_plugin_by_type('assignfeedback', 'criteria');
        if ($comments = $plugin->get_feedback_comments($exportdata->get_pluginobject()->id)) {
            ...
            foreach ($criteria as $key => $criterion) {
                if (!empty($comments[$key]) && !empty($comments[$key]->commenttext)) {
                    ...
                    $data->criteriondescpos = format_text($criterion->descriptionpos, FORMAT_PLAIN, ['context' => $context]);
					$data->criteriondescneg = format_text($criterion->descriptionneg, FORMAT_PLAIN, ['context' => $context]);
                    $data->commenttext = format_text($comments[$key]->commenttext, $comments[$key]->commentformat,
                            ['context' => $context]);
                    writer::with_context($context)->export_data(array_merge($subcontext, [$key]), $data);
                }
            }
        }
    }
    ...
    public static function delete_feedback_for_grade(assign_plugin_request_data $requestdata) {
        global $DB;
        $DB->delete_records('assignfeedback_criteria', ['assignment' => $requestdata->get_assign()->get_instance()->id,
